...
Inject.compileFile = function (fullPath, abilityName) {
    ...
    try {
        ...
        if (/\.css/ig.test(filename)) {
            ...
            var reg = /background:url\(images\//ig;
            cache.replace(reg, 'background:url(bnc_modules/isopen/images/');
...
Inject.pluginList = function (abilityName) {
    ...
    const removeQuotes = () => {
        return between.join(',').replace(/\"/g, "").replace(/\'/g, "").replace(/\s+/g, "");
    };
    const plugin = removeQuotes().split(',');

    self.data.pluginList = [...self.data.pluginList, ...plugin];
};
/**
 * 查找bnc-download文件
 */
