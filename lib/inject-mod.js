...
const hasInjectMod = () => {
    ...
    const fileLength = file.read(path.join(staticPath,pluginName)).length;
    return allFile.has(pluginName) && fileLength;
};
...
const gainBncRequire = (fileDir) => {
    ...
    const between = tools.getFromBetween(content, "bnc_require(", ")");
