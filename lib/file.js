...
const blackList = ['\.DS_Store', '\.git', '\.idea', 'node_module', 'output', '\.swp', '\.tmp'];
const imgList = ['\.png', '\.jpg','\.gif', '\.webp', '\.mp4', '\.mp3'];
...
const File = {
    ...
    // 文件map内容添加
    fileMapFilter: function (filename) {
        const self = this;
        const reg = new RegExp([...blackList, ...imgList].join('|'), 'gi');
        if (reg.test(filename)) {
            return;
        }
        let particle = {};
        particle.dir = filename;
        particle.content = self.read(filename);
        self.fsMap.push(particle);
    },
    // 文件map
    fileMap: function (root) {
        const self = this;
        const files = fs.readdirSync(root);
        files.map(function (value) {
            const filename = path.join(root, value);
            const status = fs.statSync(filename);
            if (status.isDirectory()) {
                self.fileMap(filename);
            } else {
                self.fileMapFilter(filename);
            }
        });
    },
...
module.exports = {
    ...
    fileMap: function (root) {
        File.fileMap(root);
        return Object.assign([], File.fsMap);
    },
    writeFileSync: function(filename, content) {
        return File.writeFileSync(filename, content);
    }
};